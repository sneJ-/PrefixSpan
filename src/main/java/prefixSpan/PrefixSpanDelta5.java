...
public class PrefixSpanDelta5 {
	...
	public static void main(String[] args) throws Exception {
		if(args.length != 4){
			...
			if(decoder != null){
				PrefixSpanDelta5 prefixSpan;
				if(decoder instanceof DummyInputDecoder){
					prefixSpan = new PrefixSpanDelta5(null,decoder,Double.parseDouble(args[2]),Integer.parseInt(args[3]));
				}else{
					prefixSpan = new PrefixSpanDelta5(args[0],decoder,Double.parseDouble(args[2]),Integer.parseInt(args[3]));
				}
				prefixSpan.run();
			}
		}
	 ...
	 * @throws Exception
	 */
	public PrefixSpanDelta5(String inputFile, InputDecoder decoder, double minSupport, int maxPatternLength) throws Exception{
	...
	/**
	 * Executes the prefixSpan algorithm using deltaIterations and String fields for joining.
	 * @throws Exception
	 */
	public void run() throws Exception{
		//initial workset for delta iteration (prefix, postfix, count, joinPrefix)
		DataSet<Tuple4<int[],int[],Long,String>> initialWorkSet = database.flatMap(new InitialWorkSetExpander());
		...
		public void flatMap(Tuple2<Long, int[]> sequence, Collector<Tuple4<int[], int[], Long, String>> out)
			...
			for(int i=0; i<sequence.f1.length; i++){
				if(!usedPatterns.contains(sequence.f1[i])){
					...
					out.collect(new Tuple4<int[],int[],Long,String>(new int[]{sequence.f1[i]},postfix,1L,""+sequence.f1[i]));
